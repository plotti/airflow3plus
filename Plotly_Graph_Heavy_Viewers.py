...
list_EPs = ['Der Bachelor', 'Die Bachelorette', 'Adieu Heimat - Schweizer wandern aus',
            'Bumann, der Restauranttester', 'Bauer, ledig, sucht ...', 'Ninja Warrior Switzerland']
# 3+ FR for EPs
dates_Bachelor = list()
dates_Bachelorette = list()
dates_AdieuHeimat = list()
dates_BLS = list()
dates_BdR = list()
# TV24 FR for EPs
dates_NJWS = list()
...
channel_EPs = ['3+', 'TV24']
dates_EPs = {'Der Bachelor': dates_Bachelor, 'Die Bachelorette': dates_Bachelorette,
             'Adieu Heimat - Schweizer wandern aus': dates_AdieuHeimat,
             'Bumann, der Restauranttester': dates_BdR, 'Bauer, ledig, sucht ...': dates_BLS,
             'Ninja Warrior Switzerland': dates_NJWS}
...
# TODO date parameter might be wrong
def filtering_max_values(df, channel, show, instances, first_runs=False):

    df = df[(df['channel'] == channel) & (df['show'] == show)]
    if channel in channel_EPs and show in list_EPs:
        dates = dates_EPs.get(show)
        if first_runs:
    ...
    for channel in list_list_shows:
        ...
        for shows in channel:
            ...
            temp_list.append(dict(label=str(show),
                                  method='update',
                                  args=[{'header': {'values': filtering_max_values(df, chan, show, 10, False).columns,
                                                    'line_color': 'darkslategray'},
                                         'cells': {'values': filtering_max_values(df, chan, show, 10, False).T.values,
                                                   'line_color': 'darkslategray',
                                                   'height': 90}},
                                        {'title': chan + ' ' + show,
                                         'width': 1950}
        ...
        if channel[0][1] in channel_EPs:
            for show in list_EPs:
                temp_list.append(dict(label=str(show) + 'FR',
                                      method='update',
                                      args=[
                                          {'header': {'values': filtering_max_values(df, channel[0][1], show, 10, True).columns,
                                                      'line_color': 'darkslategray'},
                                           'cells': {'values': filtering_max_values(df, channel[0][1], show, 10, True).T.values,
                                                     'line_color': 'darkslategray',
                                                     'height': 90}},
                                          {'title': channel[0][1] + ' ' + show,
                                           'width': 1950}
                                          ]
                                      )
                                 )
