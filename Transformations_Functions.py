...
def to_xl_multi_sheet(df, f, group_chans):
    ...
    for frame, chan in zip(vorabend, group_chans):
        frame = frame.T
        frame.to_excel(writer, sheet_name=chan + "_vorabend")  # send df to writer
        worksheet = writer.sheets[chan + "_vorabend"]  # pull worksheet object
    ...
    for frame, chan in zip(primetime, group_chans):
        frame = frame.T
        frame.to_excel(writer, sheet_name=chan + "_primetime")  # send df to writer
        worksheet = writer.sheets[chan + "_primetime"]  # pull worksheet object
        for idx, col in enumerate(frame):  # loop through all columns
            series = frame[col]
            max_len = max((series.astype(str).map(len).max(), len(str(series.name)))) + 1
            worksheet.set_column(idx, idx, max_len)
